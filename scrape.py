...
import pandas as pd    #python ma , lekhna mildaina so namilera aako data (Price ma 12,000) eslai milauchha
import requests    #internet ma gayera data halne kaam
from bs4 import BeautifulSoup     #html tag haru lai parse garne kaam

FILE_NAME = "today_prices.json"
URL = "https://www.sharesansar.com/today-share-price"

float_columns = [       #column haru float ma hunu parchha tara wuta data string ma chha jun pandas le milaauchha
    "Conf.",
    "Low",
    "High",
    "Open",
...
class ShareSansarScrapper:
    ...
    def fetch_data(self) -> BeautifulSoup:     #mathi ko url ko html of content lyauchha 
        response = self.fetch_html(URL)
        soup = BeautifulSoup(response, "html.parser")     #suru ko html response j chha tyo pathaune
        return soup     #beautifulsoup ko data lai return garchha

    def parse_columns(self, soup: BeautifulSoup) -> list:    #main column jhikchha
    ...
    def update_today_price(self):
        soup = self.fetch_data()    #fetchdata le html lyaune kaam garchha
        today_prices = self.parse_data(soup)      #html content bata json format ma lagne kaam garchha
        return today_prices
